...
The stack has four tightly-integrated components: a userspace NFQUEUE firewall daemon (\texttt{fw\_nfq}), a telemetry agent (\texttt{fw\_agent}), an AES-encrypted ingest receiver (\texttt{soc\_receiver}), and a real-time web dashboard. These are packaged as isolated Docker containers.
...
AES Encryption       & All agent-to-SOC telemetry encrypted with symmetric AES-256-CBC \\
...
Telemetry     & fw\_agent        & AES-encrypt; send instantly (WARNING+) or batch (INFO) \\
Transport     & TCP Port 1113    & Encrypted channel to the SOC container \\
Ingest        & soc\_receiver    & epoll loop, decrypts AES payloads, writes to SQLite \\
...
Both containers share the Docker bridge network. Service discovery uses container names as DNS hostnames. Agent UUIDs are stored in a shared volume at \texttt{./keys}.
...
Encrypted payloads received from verified agents are processed in-memory using OpenSSL libcrypto's AES-256-CBC implementation. The 16-byte initialization vector (IV) is extracted from the beginning of the incoming payload, and decryption is performed symmetrically using the pre-shared key.

Decrypted telemetry is verified against buffer limits and inserted into the PostgreSQL database. Registered web browsers connected to the dashboard are simultaneously notified using Server-Sent Events (SSE).
...
\vspace{1.5em}
\textbf{7. AES ENCRYPTED TRANSPORT PROTOCOL}\\
------------------------------------------------------------\\
The server-agent network link is encrypted end-to-end. Communication uses symmetric AES-256-CBC encryption.

\textbf{7.1 Cryptographic Parameters}\\
The symmetric parameters are structured below:

\begin{center}
\begin{tabular}{C{1.0cm} L{3.5cm} L{10.0cm}}
\hline
\thfmt{No.} & \thfmt{Parameter} & \thfmt{Action} \\
\hline
1 & Algorithm & AES-256-CBC (Advanced Encryption Standard) \\
2 & Key Size & 256 bits (32 bytes) pre-shared key \\
3 & IV Generation & Cryptographically random 16 bytes generated per packet \\
4 & Decryption & Sourced directly from the prepended 16-byte header payload \\
\hline
\end{tabular}
\end{center}

\textbf{7.2 Frame Structure}\\
The low-level network header prepended to the AES ciphertext matches the following protocol struct:
...
struct soc_msg_hdr {
    ...
    uint32_t payload_len;     /* Length of the following AES ciphertext     */
};
\end{codeblock}
