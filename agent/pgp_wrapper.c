...
int pgp_encrypt(const void *plaintext, size_t plaintext_len, char **ciphertext, size_t *ciphertext_len) {
    ...
    if (pid == 0) {
        ...
        // Execute gpg
        // --encrypt: encrypt to recipient
        // --recipient soc@soc.local: specifies the recipient key ID/email
        // --trust-model always: skips interactive trust validation
        // --armor: produces ASCII armored output
        ...
        // Write plaintext to gpg's stdin
        ...
        close(in_pipe[1]); // Close to trigger EOF in gpg

        // Read ciphertext from gpg's stdout
...
int pgp_decrypt(const char *ciphertext, size_t ciphertext_len, void **plaintext, size_t *plaintext_len) {
    ...
    if (pid == 0) {
        // Child Process: execute gpg decrypt
        dup2(in_pipe[0], STDIN_FILENO);
        dup2(out_pipe[1], STDOUT_FILENO);
        ...
        close(in_pipe[1]); // Close to trigger EOF

        // Read decrypted payload from gpg stdout
        size_t capacity = 4096;
        size_t total_read = 0;
